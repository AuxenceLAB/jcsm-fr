...
- **PWA**: `manifest.json` + `sw.js` (cache v109). Strategies: network-first for public HTML, cache-first for assets. Private API and report paths are network-only and never stored.
...
After CSS/JS changes, bump the cache version in `sw.js` (comment + `STATIC_CACHE`, `DYNAMIC_CACHE`) to invalidate old caches. Current version: **v109**. There is no private API cache.

Also bump `version` in `js/config.js`. Current: **2.57.22** (HTML `?v=109`).
