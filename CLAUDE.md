...
- **PWA**: `manifest.json` + `sw.js` (cache v77). Strategies: network-first for HTML/API, cache-first for assets.
...
After CSS/JS changes, bump the cache version in `sw.js` (lines 1-4: comment + `STATIC_CACHE`, `DYNAMIC_CACHE`, `API_CACHE`) to invalidate old caches. Current version: **v77**.
