...
- **PWA**: `manifest.json` + `sw.js` (cache v108). Strategies: network-first for public HTML, cache-first for assets. Private API and report paths are network-only and never stored.

### Current public layout contract (2026-09-05)

`scripts/public-pages.cjs` is the catalogue of the 154 shared public pages. `css/public-layout.css` is linked last in each head, scoped to `body.jcsm-public`. It takes precedence over the historical B1 geometry described above: one 80rem shell, 16px mobile / 24px desktop gutters, an 80px navigation bar and a 40px historical logo. The header offset is applied once, not again after breadcrumbs. Marketing heroes share left-aligned typography. The 51 articles retain a 48rem reading column and the six legal pages retain 56rem. Do not add automatic padding to nested `max-w-*` utilities. The legacy automatic gutters remain only on pages outside this public contract.

Palette (2026-09): brand blue `#205BC4` / `#19499F` / `#153C80`, graphite `#14181C`, steel `#5A6470`; the Tailwind `blue-*` utilities are mapped to these in `tailwind.config.js`, so `#2563EB` and gradients are retired. Unverifiable performance figures are guarded by `scripts/proof-claims.test.cjs`.

After adding a public page, run `node scripts/sync-public-layout.cjs --write` and verify the catalogue and `scripts/public-layout.test.cjs`. Private pages, partner prototypes and `demo/` are excluded. Do not copy the old blog header height or logo dimensions.

### JavaScript Modules (`js/`)

| File | Role |
|------|------|
| `config.js` | API endpoints (all server-side proxied), auth helpers (session CRUD, Bearer token headers), cache TTLs. Version 2.57.21. |
...
After CSS/JS changes, bump the cache version in `sw.js` (comment + `STATIC_CACHE`, `DYNAMIC_CACHE`) to invalidate old caches. Current version: **v108**. There is no private API cache.

Also bump `version` in `js/config.js`. Current: **2.57.21** (HTML `?v=108`).
