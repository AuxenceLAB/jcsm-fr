...
- **PWA**: `manifest.json` + `sw.js` (cache v111). Strategies: network-first for public HTML, cache-first for assets. Private API and report paths are network-only and never stored.
...
| File | Role |
|------|------|
| `config.js` | API endpoints (all server-side proxied), auth helpers (session CRUD, Bearer token headers), cache TTLs. Version 2.57.24. |
...
After CSS/JS changes, bump the cache version in `sw.js` (comment + `STATIC_CACHE`, `DYNAMIC_CACHE`) to invalidate old caches. Current version: **v111**. There is no private API cache.

Also bump `version` in `js/config.js`. Current: **2.57.24** (HTML `?v=111`).
