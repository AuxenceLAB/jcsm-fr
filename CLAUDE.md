...
- **PWA**: `manifest.json` + `sw.js` (cache v99). Strategies: network-first for public HTML, cache-first for assets. Private API and report paths are network-only and never stored.
...
After CSS/JS changes, bump the cache version in `sw.js` (comment + `STATIC_CACHE`, `DYNAMIC_CACHE`) to invalidate old caches. Current version: **v99**. There is no private API cache.
